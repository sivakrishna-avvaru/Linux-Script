...
#!/bin/bash -x
################################################################################
#	Author   : Abhilash Dd Chaitanya                                       #
#	Email    : achaitan@in.ibm.com                                         #
#	Reviewed and Modified : Ashutosh Mishra(ashmishn@in.ibm.com	       #
#	Platform : Linux (RHEL6 and RHEL7)						       #
#	Script   : Shell script					               #
...
sk2=`cat /etc/rsyslog.conf | grep "^*.info;mail.none;authpriv.none;cron.none" |grep /var/log/messages |wc -c`
	...
	then
		...
		then
				...
				echo "$c" >>p5
				echo "$z" >>p6
				echo "AD.1.2.1.3:AD.1.2.1.4" >>p12
...
if [ $sl == weekly ] && [ $sn -ge $LOG_ROTATE_WEEK ]
then
...
#AD.1.1.1.2:Fifth field of /etc/shadow
...
#AD.1.1.2.1:2nd field of /etc/shadow
...
#AD.1.1.2.2:2nd field of /etc/passwd
...
#AD.1.1.6.0:loginretries value in password-auth and system-auth
...
#AD.1.1.3.2:4th field of /etc/shadow
cat /etc/passwd | egrep -v "/sbin/nologin|sync|shutdown|halt|/bin/false" | awk -F":" '{print $1}' >temp_passwd
for i in `cat temp_passwd`
do
...
#AD.1.1.12.1:2nd field of /etc/shadow
...
#AD.1.1.11.1;AD.1.1.13.1.0:Non-expiring ID's
for i in `cat /etc/passwd | egrep -v "/sbin/nologin|sync|shutdown|halt|/bin/false" | awk -F":" '{print $1}'`
do
...
then
	sk1=`passwd -S $i |awk '{print $2}'`
		...
		echo "$c" >>p5
		echo "$z" >>p6
		echo "AD.1.1.11.1;AD.1.1.13.1.0" >>p12
	else
		echo "Password Requirementss" >>p1
		echo "direct_or_remote_login" >>p2
...
#AD.2.1.3.0:AD.2.1.3.1:AD.2.1.3.2
...
sk=`cat $i |egrep 'md5|sha512' |grep shadow |wc -l`
	...
	then
		...
		echo "yes" >>p4
		echo "AD.2.1.3.0:AD.2.1.3.1:AD.2.1.3.2" >>p12
		echo "$c" >>p5
		echo "$z" >>p6
	else
		echo "Encryption" >>p1
...
#AD.1.1.9.0:AD.1.1.9.1:non-expiry-passwords
...
then
	...
	then
		...
		echo "$c" >> p5
		echo "$z" >>p6
		echo "AD.1.1.9.0:AD.1.1.9.1" >>p12
	else
		echo "Password Requirementss" >>p1
		echo "Non-expiring passwords" >>p2
...
echo "This cann't be health checked because the shared ID root should be vaulted in UAT or other similar tool and accessed through Check-out and Check-in process" >>p3
...
then
		...
		echo "AD.1.5.2.1,AD.1.5.2.2" >>p12
...
#AD.1.1.13.3,AD.1.1.10.2:FTP filecheck
ftpRPM=`rpm -q vsftpd`
if [ $? -ne 0 ]
then
		echo "Password Requirementss" >>p1
...
then
	...
	then
		...
		if [ -f /etc/pam.d/login ] || [ -f /etc/pam.d/passwd ] || [ -f  /etc/pam.d/sshd ] || [ -f /etc/pam.d/su ]
		then	
...
#AD.1.4.4.:2nd field of /etc/shadow
cat /etc/shadow | awk -F":" '{print $1 "   "  $2}'| grep -v '!!'| grep -v '*'| grep -v '!' | awk -F "  " '{print $1}' >temp_shadow2
for i in `cat temp_shadow2`
do
	sk1=`passwd -S $i |awk '{print $11}' |cut -c1-5`
	if [ "$sk1" == "crypt" ]
        then
		echo "System Settings" >>p1
                echo "/etc/shadow-Encrypted Password" >>p2
		echo "Password-is-encrypted-for-user-$i" >>p3
		echo "yes" >>p4
		echo "AD.1.4.4" >>p12
		echo "$c" >> p5
		echo "$z" >>p6
		...
		echo "System Settings" >>p1
                echo "/etc/shadow-Encrypted Password" >>p2
		echo "Password-is-not-encrypted-for-user-$i" >>p3
		echo "AD.1.4.4" >>p12
		echo "no" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
...
sp1=`cat /etc/snmp/snmpd.conf |grep ^rwcommunity |grep -i private |wc -l`
	if [ $sk -gt 0 ] || [ $sk1 -gt 0 ] || [ $sp -gt 0 ] || [ $sp1 -gt 0 ]
	then
		echo "Network Settings" >>p1
		...
		echo "$c" >>p5
		echo "$z" >>p6
		echo "AD.1.5.9.18;AD.1.5.9.19" >>p12
	else
		echo "Network Settings" >>p1
		echo "SNMP Service" >>p2
...
then
	...
	then
		...
		echo "$c" >>p5
		echo "$z" >>p6
		echo "AD.1.5.10.2:AD.1.5.11" >>p12
	else
		echo "Network Settings" >>p1
		echo "NIS and NIS+ maps" >>p2
...
then
	...
	sm=`getent group $sk |awk -F: '{print $3}'`
		...
		then
			...
			echo "$c" >>p5
			echo "$z" >>p6
			echo "AD.1.8.1.2,AD.1.8.1.3" >>p12
	fi
	fi
else
			echo "Protecting Resources - OSRs" >>p1
...
fz2=`ls -l /etc/vsftpd/vsftpd.conf |wc -c`
	...
	then
		...
		then
			...
			echo "AD.1.4.5:AD.1.5.1.1:AD.1.5.1.2:AD.1.5.1.3:AD.1.5.1.4:AD.1.5.1.5:AD.1.5.1.6:AD.1.5.1.7:AD.1.5.1.8">>p12
			echo "no" >>p4
			echo "$c" >>p5
			echo "$z" >>p6
		else
...
#AD.1.8.8:AD.1.8.9:AD.1.8.12.1.1:AD.1.8.12.1.2:AD.1.8.12.2:AD.1.8.12.3:AD.1.8.12.4:AD.1.8.13.3:AD.1.8.13.4:AD.1.8.14.2:AD.1.8.14.3:AD.1.8.15.2:AD.1.8.15.3:AD.1.8.17.2:AD.1.8.17.3:AD.1.8.18.2:AD.1.8.18.3:AD.1.8.19.2:AD.1.8.19.3:AD.1.8.20.2:AD.1.8.20.3:AD.1.9.1.1:AD.1.8.21.2:AD.1.8.21.3:AD.1.8.22.1:AD.1.8.22.2:AD.1.8.22.3:AD.1.8.22.4:AD.1.8.13.1.2
...
do
	...
	echo "$c" >> p5
	echo "$z" >>p6
	echo AD.1.8.8:AD.1.8.9:AD.1.8.12.1.1:AD.1.8.12.1.2:AD.1.8.12.2:AD.1.8.12.3:AD.1.8.12.4:AD.1.8.13.3:AD.1.8.13.4:AD.1.8.14.2:AD.1.8.14.3:AD.1.8.15.2:AD.1.8.15.3:AD.1.8.17.2:AD.1.8.17.3:AD.1.8.18.2:AD.1.8.18.3:AD.1.8.19.2:AD.1.8.19.3:AD.1.8.20.2:AD.1.8.20.3:AD.1.9.1.1:AD.1.8.21.2:AD.1.8.21.3:AD.1.8.22.1:AD.1.8.22.2:AD.1.8.22.3:AD.1.8.22.4:AD.1.8.13.1.2 >>p12
done
...
then
	...
	if (( $($BC <<< "$sz<7") > 0 ))
	then
		sk=`ls -l /var/log/messages |awk '{print $4}'`
		sj=`getent group $sk |awk -F: '{print $3}'`
		if [ $sz -le 99 ] || [[ $sz -ge 101 && $sz -le 499 ]]
		then
			ss1=`find /var/log/messages -type f -perm /g+r |wc -l`
			ss2=`find /var/log/messages -type f -perm /g+w |wc -l`
			if [ $ss1 -gt 0 ] || [ $ss2 -gt 0 ]
			then
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/messages-Group Read-Write access" >>p2
				echo "/var/log/messages has correct permission and owned by priviledged group ID" >> p3
				echo "Yes" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.2" >>p12
			else
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/messages-Group Read-Write access" >>p2
				echo "/var/log/messages has incorrect permission and not owned by priviledged group ID" >> p3
				echo "no" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.2" >>p12
			...
			echo "Protecting Resources - OSRs" >>p1
			echo "/var/log/messages-Group Read-Write access" >>p2
			echo "/var/log/messages not owned by priviledged group ID" >> p3
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			echo "AD.1.8.12.2" >>p12
	...
	then
		sk=`ls -l /var/log/messages |awk '{print $4}'`
		sj=`getent group $sk |awk -F: '{print $3}'`
		if [ $sj -le 99 ] || [[ $sj -ge 101 && $sj -le 999 ]]
		then
			ss1=`find /var/log/messages -type f -perm /g+r |wc -l`
			ss2=`find /var/log/messages -type f -perm /g+w |wc -l`
			if [ $ss1 -gt 0 ] || [ $ss2 -gt 0 ]
...
then
	...
	if (( $($BC <<< "$sz<7") > 0 ))
	then
		sk=`ls -l /var/log/wtmp |awk '{print $4}'`
		sj=`getent group $sk |awk -F: '{print $3}'`
		if [ $sj -le 99 ] || [[ $sj -ge 101 && $sj -le 499 ]]
		then
			ss1=`find /var/log/wtmp -type f -perm /g+r |wc -l`
			ss2=`find /var/log/wtmp -type f -perm /g+w |wc -l`
			if [ $ss1 -gt 0 ] || [ $ss2 -gt 0 ]
			then
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/wtmp-Group Read-Write access" >>p2
				echo "/var/log/wtmp has correct permission and owned by priviledged group ID" >> p3
				echo "Yes" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.3" >>p12
			else
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/wtmp-Group Read-Write access" >>p2
				echo "/var/log/wtmp has incorrect permission and not owned by priviledged group ID" >> p3
				echo "no" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.3" >>p12
			...
			echo "Protecting Resources - OSRs" >>p1
			echo "/var/log/wtmp-Group Read-Write access" >>p2
			echo "/var/log/wtmp not owned by priviledged group ID" >> p3
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			echo "AD.1.8.12.3" >>p12
...
then
	...
	if (( $($BC <<< "$sz<7") > 0 ))
	then
		sk=`ls -l /var/log/faillog |awk '{print $4}'`
		sj=`getent group $sk |awk -F: '{print $3}'`
		if [ $sj -le 99 ] || [[ $sj -ge 101 && $sj -le 499 ]]
		then
			ss1=`find /var/log/faillog -type f -perm /g+r |wc -l`
			ss2=`find /var/log/faillog -type f -perm /g+w |wc -l`
			if [ $ss1 -gt 0 ] || [ $ss2 -gt 0 ]
			then
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/faillog-Group Read-Write access" >>p2
				echo "/var/log/faillog has correct permission and owned by priviledged group ID" >> p3
				echo "Yes" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.1.1" >>p12
			else
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/faillog-Group Read-Write access" >>p2
				echo "/var/log/faillog has incorrect permission and not owned by priviledged group ID" >> p3
				echo "no" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.1.1" >>p12
			...
			echo "Protecting Resources - OSRs" >>p1
			echo "/var/log/faillog-Group Read-Write access" >>p2
			echo "/var/log/faillog not owned by priviledged group ID" >> p3
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			echo "AD.1.8.12.1.1" >>p12
...
then
	...
	if (( $($BC <<< "$sz<7") > 0 ))
	then
		sk=`ls -l /var/log/tallylog |awk '{print $4}'`
		sj=`getent group $sk |awk -F: '{print $3}'`
		if [ $sj -le 99 ] || [[ $sj -ge 101 && $sj -le 499 ]]
		then
			ss1=`find /var/log/tallylog -type f -perm /g+r |wc -l`
			ss2=`find /var/log/tallylog -type f -perm /g+w |wc -l`
			if [ $ss1 -gt 0 ] || [ $ss2 -gt 0 ]
			then
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/tallylog-Group Read-Write access" >>p2
				echo "/var/log/tallylog has correct permission and owned by priviledged group ID" >> p3
				echo "Yes" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.1.2" >>p12
			else
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/tallylog-Group Read-Write access" >>p2
				echo "/var/log/tallylog has incorrect permission and not owned by priviledged group ID" >> p3
				echo "no" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.1.2" >>p12
			...
			echo "Protecting Resources - OSRs" >>p1
			echo "/var/log/tallylog-Group Read-Write access" >>p2
			echo "/var/log/tallylog not owned by priviledged group ID" >> p3
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			echo "AD.1.8.12.1.2" >>p12
...
then
	...
	if (( $($BC <<< "$sz<7") > 0 ))
	then
		sk=`ls -l /var/log/secure |awk '{print $4}'`
		sj=`getent group $sk |awk -F: '{print $3}'`
		if [ $sj -le 99 ] || [[ $sj -ge 101 && $sj -le 499 ]]
		then
			ss1=`find /var/log/secure -type f -perm /g+r |wc -l`
			ss2=`find /var/log/secure -type f -perm /g+w |wc -l`
			if [ $ss1 -gt 0 ] || [ $ss2 -gt 0 ]
			then
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/secure-Group Read-Write access" >>p2
				echo "/var/log/secure has correct permission and owned by priviledged group ID" >> p3
				echo "Yes" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.4" >>p12
			else
				echo "Protecting Resources - OSRs" >>p1
				echo "/var/log/secure-Group Read-Write access" >>p2
				echo "/var/log/secure has incorrect permission and not owned by priviledged group ID" >> p3
				echo "no" >>p4
				echo "$c" >> p5
				echo "$z" >>p6
				echo "AD.1.8.12.4" >>p12
			...
			echo "Protecting Resources - OSRs" >>p1
			echo "/var/log/secure-Group Read-Write access" >>p2
			echo "/var/log/secure not owned by priviledged group ID" >> p3
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			echo "AD.1.8.12.4" >>p12
...
do
	...
	if [ $str -eq 0 ]
		...
		echo "AD.1.8.13.3;AD.1.8.13.4" >>p12
		echo "yes" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
	else
...
do
	...
	if [ $str -eq 0 ]
		...
		echo "AD.1.8.14.2;AD.1.8.14.3" >>p12
		echo "yes" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
	else
...
do
	...
	if [ "$str" != "w" ]
		...
		echo "AD.1.8.15.2;AD.1.8.15.3" >>p12
		echo "yes" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
	else
...
then
	...
	do
		...
		then
			...
			echo "AD.1.8.17.2;AD.1.8.17.3" >>p12
			echo "yes" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
		else
...
if [ $? -eq 0 ]
then
	sz=`cat /etc/login.defs |grep -v '#' |grep -i umask |uniq |wc -l`
	if [ $sz -eq 1 ]
	then
		sk=`cat /etc/login.defs |grep -v '#' |grep -i umask |awk '{print $2}'`
		if [ "$sk" == "$UMASK_VAL" ]
		then
			echo "Protecting Resources - User Resources" >>p1
               		echo "New-user-home-directory-permission" >>p2
			echo "umask-value-set-as-$sk-in-/etc/login.defs-and-default-permission-of-user-home-directory-permission-set-correct" >>p3
			echo "AD.1.9.1.1" >>p12
			echo "yes" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			...
			echo "Protecting Resources - User Resources" >>p1
                	echo "New-user-home-directory-permission" >>p2
			echo "umask-value-set-incorrect-as-$sk-in-/etc/login.defs-and-default-permission-of-user-home-directory-permission-set-incorrect" >>p3
			echo "AD.1.9.1.1" >>p12
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			...
			echo "Protecting Resources - User Resources" >>p1
                	echo "New-user-home-directory-permission" >>p2
			echo "UMASK-value-is-set-more-than-one-in-/etc/login.defs" >>p3
			echo "AD.1.9.1.1" >>p12
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
			...
			echo "Protecting Resources - User Resources" >>p1
                	echo "New-user-home-directory-permission" >>p2
			echo "UMASK-value-is-not-set-in-/etc/login.defs-for-default-permission-of-user-home-directory" >>p3
			echo "AD.1.9.1.1" >>p12
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
...
then
	...
	then
		...
		echo "no" >> p4
		echo "AD.2.1.1,AD.2.1.2,AV.2.1.1.2,AV.2.1.1.3,AV.2.1.1.4,AV.1.7.2" >>p12
		echo "$c" >> p5
		echo "$z" >>p6	
	else
		echo "Encryption" >>p1
...
then
	...
	do
		...
		then
			...
			echo "yes" >>p4
			echo "AD.1.5.8.1,AD.1.5.8.2,AD.1.5.8.3,AD.1.5.8.4,AD.1.5.8.5,AD.1.5.8.6,AD.1.5.8.7,AD.1.5.8.8,AD.1.5.9.1,AD.1.5.9.2,AD.1.5.9.3,AD.1.5.9.4,AD.1.5.9.5,AD.1.5.9.6,AD.1.5.9.7,AD.1.5.9.8,AD.1.5.9.9,AD.1.5.9.10,AD.1.5.9.11,AD.1.5.9.12,AD.1.5.9.13,AD.1.5.9.14,AD.1.5.9.15,AD.1.5.9.16,AD.1.5.9.17" >>p12
			echo "$c" >>p5
			echo "$z" >>p6
		else
			echo "Network Settings" >>p1
...
then
	A=`getenforce`
	if [ "$A" == "Enforcing" ] || [ "$A" == "Permissive" ]
	then
		echo "system-settings" >>p1
		echo "SELINUX-policy-check" >>p2
		echo "$A-enabled" >>p3
		echo "yes" >>p4
		echo "$c" >>p5
		echo "$z" >>p6
		...
		echo "system-settings" >>p1
		echo "SELINUX-policy-check" >>p2
		echo "Enforcing/Permissive-not-enabled" >>p3
		echo "no" >>p4
		echo "$c" >>p5
		echo "$z" >>p6
		echo "AD.1.4.3.1" >>p12
	...
	then
				echo "system-settings" >>p1
				echo "SELINUX-policy-check" >>p2
				echo "Enforcing/Permissive-enabled" >>p3
				echo "yes" >>p4
				echo "$c" >>p5
				echo "$z" >>p6
				echo "AD.1.4.3.1" >>p12
	...
	rpm -qa | grep -i "AppArmor"
		...
		then
				...
				echo "system-settings" >>p1
				echo "SELINUX-policy-check" >>p2
				echo "SELINUX-APPARMOR-NOT-ENABLED" >>p3
				echo "no" >>p4
				echo "$c" >>p5
				echo "$z" >>p6
				echo "AD.1.4.3.1" >>p12	
...
then
	...
	echo "$c" >>p5
	echo "$z" >>p6
	echo "AD.1.4.4.1:AD.1.4.4.2" >>p12
...
#AD.30.0.1.4:Screen timeut inactivity
if [ -f /etc/profile.d/IBMsinit.csh ]
then
...
#AD.30.0.1.5:auto time out check
echo "/etc/skel/.cshrc,/etc/skel/.login,/etc/skel/.profile,/etc/skel/.bashrc,/etc/skel/.bash_profile,/etc/skel/.bash_login,/etc/skel/.tcshrc" > temp
tr "," "\n" < temp > temp1
for i in `cat temp1`
do
if [ -f $i ]
then
	sk=`cat $i | grep -i "TMOUT" | wc -l`
	if [ $sk -gt 0 ]
	then
		echo "Password Requirements" >>p1
                echo "OS Automatic Logoff skeleton files" >>p2
		echo "OS Automatic Logoff is set in skeleton file $i" >>p3
		echo "AD.30.0.1.5">>p12
		echo "yes" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
	else
		echo "Password Requirements" >>p1
                echo "OS Automatic Logoff skeleton files" >>p2
		echo "OS Automatic Logoff is not set in skeleton file $i" >>p3
		echo "AD.30.0.1.5">>p12
		echo "no" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
		...
		echo "Password Requirements" >>p1
                echo "OS Automatic Logoff skeleton files" >>p2
		echo "OS skeleton files not exist" >>p3
		echo "AD.30.0.1.5">>p12
		echo "Not_Applicable" >>p4
		echo "$c" >> p5
		echo "$z" >>p6
...
for i in `cat /etc/passwd |egrep -v "/sbin/nologin|sync|shutdown|halt|/bin/false" | awk -F":" '{print $1}'`
do
sk=`getent passwd $i | awk -F":" '{print $7}'`
...
then
	...
	then
		sk=`cat /etc/rsyslog.conf |grep -v '#' |grep "'*.*.*@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		sl=`cat /etc/rsyslog.conf |grep -v '#' |grep "'*.*.*@@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		sk1=`cat /etc/rsyslog.conf |grep -v '#' |grep "'auth.*.*@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		sl1=`cat /etc/rsyslog.conf |grep -v '#' |grep "'auth.*.*@@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		if [ $sk -gt 0 ] || [ $sl -gt 0 ] || [ $sk1 -gt 0 ] || [ $sl1 -gt 0 ]
		then
			echo "Logging" >>p1
		        echo "Logging in external system" >>p2
			echo "Remote syslog server IP and Port is available in /etc/rsyslog.conf" >>p3
			echo "AD.30.0.2.1">>p12
			echo "yes" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
		else
			echo "Logging" >>p1
		        echo "Logging in external system" >>p2
			echo "Remote syslog server IP and Port is not available in /etc/rsyslog.conf" >>p3
			echo "AD.30.0.2.1">>p12
			echo "no" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
...
then
	...
	then
		sk=`cat /etc/rsyslog.conf |grep -v '#' |grep "'*.*.*@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		sl=`cat /etc/rsyslog.conf |grep -v '#' |grep "'*.*.*@@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		sk1=`cat /etc/rsyslog.conf |grep -v '#' |grep "'authpriv.*.*@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		sl1=`cat /etc/rsyslog.conf |grep -v '#' |grep "'authpriv.*.*@@$SYSLOG_IP:$SYSLOG_PORT" |wc -l`
		if [ $sk -gt 0 ] || [ $sl -gt 0 ] || [ $sk1 -gt 0 ] || [ $sl1 -gt 0 ]
		then
			echo "Logging" >>p1
		        echo "Logging in external system" >>p2
			echo "Remote syslog server IP and Port is available in /etc/rsyslog.conf" >>p3
			echo "AD.30.0.2.2">>p12
			echo "yes" >>p4
			echo "$c" >> p5
			echo "$z" >>p6
		else
...
paste p6 p12 p1 p2 p3 p4 p5 > `hostname`_Linux_SSH_SUDO$c_mhc.csv
chmod 644 `hostname`_Linux_SSH_SUDO$c_mhc.csv
rm -rf temp_shadow temp_shadow1 temp1_shadow temp_shadow2 temp_shadow3 temp-ud psw_temp temp_uid temp_uid1 temp_gid temp_gid1 pasd_temp p5 p4 p3 p2 p1 p6 p12 f1 t1 temp_pam.so world-writable-test log_file1
...
do
	...
	then
		...
		then
			...
			echo "Identify and Authenticate Users" >>p1
			echo "PermitRootLogin without-password" >>p2
			echo "no-authorized-keys-found-no-root-verification-required" >>p3
			echo "$c" >>p5
			echo "$z" >>p6
			echo "yes" >>p4
			echo "AV.1.7.1.2" >>p12
...
paste p6 p12 p1 p2 p3 p4 p5 >> `hostname`_Linux_SSH_SUDO$c_mhc.csv
chmod 644 `hostname`_Linux_SSH_SUDO$c_mhc.csv
rm -rf p5 p4 p3 p2 p1 p6 p12 f1 temp_id
...
then
		...
		echo "$c" >>p5
		echo "$z" >>p6
		echo "ZY.1.4.2.0,ZY.1.4.2.1" >>p12
else
		echo "System Settings" >>p1
		echo "Commands which allow shell escape" >>p2
...
then
	...
	echo "$c" >>p5
	echo "$z" >>p6
	echo "ZY.1.4.3.1,ZY.1.8.2.1,ZY.1.8.2.2,ZY.1.8.1.2,ZY.1.8.1.4" >>p12
else
	echo "System Settings" >>p1
	echo "Specific commands/programs executed via sudo:" >>p2
	...
	do
		...
		then
			...
			echo "File permission is valid for $i" >>p3
		        ...
		        echo "$c" >>p5
		        echo "$z" >>p6
		        echo "ZY.1.8.1.1,ZY.1.8.1.3,ZY.1.8.1.5,ZY.1.8.1.6,ZY.1.8.2.3" >>p12
		else
			echo "Protecting Resources - OSRs" >>p1
		        echo "File permission in /etc/sudoers.d and /etc/sudoers" >>p2
...
paste p6 p12 p1 p2 p3 p4 p5 >> `hostname`_Linux_SSH_SUDO$c_mhc.csv
rm -rf p5 p4 p3 p2 p1 p6 p12 temp_sid file1
chmod 644 `hostname`_Linux_SSH_SUDO$c_mhc.csv
